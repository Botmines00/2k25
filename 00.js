...
(() => {
  ...
  const criarTile = (numero) => {
    ...
    tile.style.cssText = `
      width: 28px; height: 28px; border-radius: 6px; /* Um pouco maior e mais arredondado */
      background-color: ${corData.cor}; display: flex;
      justify-content: center; align-items: center;
      font-size: 14px; font-weight: bold; /* Fonte um pouco maior */
      color: ${corData.texto}; margin: 0 3px; /* Mais espaçamento entre os tiles */
  ...
  const mostrarResultadoFinal = (tipo) => {
    ...
    box.style.cssText = `
      ...
      background: ${bgColor}; color: white; padding: 8px 16px; font-weight: bold; /* Mais padding */
      border-radius: 8px; font-family: sans-serif; z-index: 999999; /* Mais arredondado */
      box-shadow: 0 0 12px ${bgColor}; /* Sombra mais destacada */
    `;
  ...
  const setupUI = () => {
    ...
    style.textContent = `
      ...
      #blazeMenu {
        ...
        background: linear-gradient(135deg, #1a1a1a 0%, #2a2a2a 50%, #1a1a1a 100%); /* Gradiente mais sutil */
        background-size: 200% 200%;
        animation: gradient-animation 10s ease infinite;
        color: white;
        font-family: 'Inter', 'Roboto', sans-serif; /* Sugerindo fontes mais profissionais */
        border-radius: 12px; /* Mais arredondado */
        box-shadow: 0 6px 20px rgba(0, 0, 0, 0.6), 0 0 10px rgba(255, 0, 0, 0.2) inset; /* Sombra mais suave e profunda */
        padding: 18px; /* Mais padding geral */
        z-index: 99999;
        box-sizing: border-box; /* Garante que padding e border não aumentem o width */
      }
      .instagramHeader {
        font-size: 13px; text-align: center; font-weight: 600; /* Mais negrito */
        background-color: rgba(0, 0, 0, 0.7); /* Mais transparente */
        padding: 6px 10px; border-radius: 6px; margin-bottom: 12px; /* Mais padding e margem */
      }
      .usernameSlider {
      ...
      #resultSmBox {
        width: 70px; height: 70px; /* Ainda maior para destaque */
        display: flex; justify-content: center; align-items: center;
        margin: 15px auto; /* Mais margem */
        border-radius: 50%;
        box-shadow: 0 0 20px rgba(255, 60, 89, 0.8); /* Sombra mais intensa para o destaque */
      }
      #resultNumberCircle {
        width: 60px; height: 60px; /* Maior */
        border-radius: 50%; display: flex; align-items: center; justify-content: center;
        font-weight: 800; font-size: 30px; /* Fonte bem maior e mais negrita */
        color: white;
      }
      #legendaResultado, #statusRoleta {
        font-size: 14px; text-align: center; margin: 6px 0; /* Mais margem */
        color: #ddd; /* Cor um pouco mais clara */
      }
      #barraGiro {
        width: 100%; height: 10px; /* Mais alta */
        background: #333; border-radius: 5px; overflow: hidden;
        margin: 15px 0; /* Mais margem */
        box-shadow: inset 0 1px 3px rgba(0,0,0,0.5); /* Sombra interna sutil */
      }
      #progressoInterno {
        width: 100%; height: 100%;
        background: linear-gradient(90deg, #ff3c59, #ff8c00); /* Gradiente na barra */
        animation: descarregar 13.5s linear forwards, gradient-animation 2s linear infinite; /* Animação de gradiente na barra */
        background-size: 200% 100%;
      }
      #sugestaoBox {
        text-align: center; font-size: 16px; padding: 10px; /* Mais padding, estilo de botão */
        margin: 15px 0; /* Mais margem */
        border-radius: 8px; font-weight: bold; color: white;
        box-shadow: 0 3px 8px rgba(0, 0, 0, 0.4);
        cursor: pointer; /* Indica que é clicável */
        transition: transform 0.2s ease; /* Transição suave ao clicar */
      }
      #sugestaoBox:active {
        transform: scale(0.98); /* Efeito de clique */
      }
      #ultimosResultados {
        display: flex; justify-content: center; margin: 15px 0 0; /* Mais margem e sem margem inferior */
        padding-top: 10px; /* Espaçamento interno */
        border-top: 1px solid rgba(255, 255, 255, 0.15); /* Separador mais visível */
      }
      .statusOnline {
        text-align: center; font-size: 12px; margin-top: 12px; /* Mais margem */
      ...
      .dotOnline {
        width: 9px; height: 9px; background-color: #00ff00; /* Um pouco maior */
  ...
  const mostrarResultadoFinal = (tipo) => {
    ...
    box.style.cssText = `
      position: fixed; top: 70px; left: 50%; transform: translateX(-50%);
      background: ${bgColor}; color: white; padding: 6px 12px; font-weight: bold;
  ...
  const setupUI = () => {
    ...
    style.textContent = `
      ...
      #blazeMenu {
        position: fixed; top: 100px; left: 20px; width: 230px;
        background: linear-gradient(135deg, #1a1a1a, #2a2a2a, #1a1a1a); /* Gradiente suave */
        background-size: 200% 200%;
        animation: gradient-animation 10s ease infinite; /* Animação do gradiente */
        color: white; font-family: sans-serif;
        border-radius: 10px;
        box-shadow: 0 4px 15px rgba(0, 0, 0, 0.5), 0 0 8px rgba(255, 0, 0, 0.3) inset; /* Sombra mais suave */
        padding: 15px; /* Mais padding geral */
        z-index: 99999;
      }
      .instagramHeader {
        font-size: 12px; text-align: center; font-weight: bold;
        background-color: rgba(0, 0, 0, 0.6); /* Mais transparente */
        padding: 4px 6px; border-radius: 4px; margin-bottom: 8px; /* Mais margem */
      }
      .usernameSlider {
        width: 100%; overflow: hidden; white-space: nowrap; box-sizing: border-box;
      }
      .usernameSlider span {
        display: inline-block; color: #00ff00; font-weight: bold;
        font-size: 14px; padding-left: 100%; animation: slide 10s linear infinite;
      }
      #resultSmBox {
        width: 60px; height: 60px; /* Maior */
        display: flex; justify-content: center; align-items: center;
        margin: 10px auto; /* Mais margem */
        border-radius: 50%; /* Círculo perfeito */
        box-shadow: 0 0 15px rgba(255, 60, 89, 0.7); /* Sombra para destaque */
      }
      #resultNumberCircle {
        width: 50px; height: 50px; /* Maior */
        border-radius: 50%; display: flex; align-items: center; justify-content: center;
        font-weight: bold; font-size: 24px; /* Fonte maior */
        color: white; /* Cor padrão para o número */
      }
      #legendaResultado, #statusRoleta {
        font-size: 13px; text-align: center; margin: 5px 0; /* Mais margem */
        color: #ccc;
      }
      #barraGiro {
        width: 100%; height: 8px; /* Mais alta */
        background: #333; border-radius: 4px; overflow: hidden;
        margin: 10px 0; /* Mais margem */
      }
      #progressoInterno {
        width: 100%; height: 100%;
        background: linear-gradient(90deg, #ff3c59, #ff8c00); /* Gradiente na barra */
      ...
      #sugestaoBox {
        text-align: center; font-size: 14px; padding: 8px; /* Mais padding */
        margin: 10px 0; /* Mais margem */
        border-radius: 6px; font-weight: bold; color: white;
        box-shadow: 0 2px 5px rgba(0, 0, 0, 0.3);
      }
      #ultimosResultados {
        display: flex; justify-content: center; margin: 10px 0; /* Mais margem */
        padding-top: 5px; /* Espaçamento interno */
        border-top: 1px solid rgba(255, 255, 255, 0.1); /* Separador sutil */
      }
      .statusOnline {
        text-align: center; font-size: 11px; margin-top: 8px; /* Mais margem */
        color: #00ff00; font-weight: bold;
        background-color: rgba(0, 0, 0, 0.6); /* Mais transparente */
